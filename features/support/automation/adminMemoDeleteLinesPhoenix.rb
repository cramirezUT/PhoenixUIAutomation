...
RSpec.describe "Admin->Memo: Admin Memo Remove Lines body functionality test", :adminMemo do
  begin
    before(:all) do
      puts "adminMemoRemoveLinesTest"
      launchToteBrowser
      selectSiteTable
      logInFunction
    end
    ...
    it "Select a memo" do
      ...
      @value = "\n" + $adminMemoNewBodyText
			adminMemoSubjectModalBodyTextArea.flash.set (@value.strip)
		end

    it "Verify Body position after Removing Extra line" do
       verifyNewMemoBodyAfterInsertText(adminMemoSubjectModalBodyTextArea.text)
    end

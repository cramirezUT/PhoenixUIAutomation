...
RSpec.describe "Admin->Seller->Edit admin Seller Seniority-P2", :sellerEdit do
  begin
    before(:all) do
      puts "adminSellerEdit Seniority-P2"
      launchToteBrowser
      selectSiteTable
      logInFunction
    end
    ...
    it "sellerSeniority" do
      sellerEdit.flash(color: ["yelllow"]).click
      sellerSeniority.flash(color: ["yelllow"])
      sellerSeniority.set ("1234567890")
      sendKeysTab
    end
